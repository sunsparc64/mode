...
def get_s3_bucket_private_url(install_access,install_secret,install_region,install_bucket,install_key)
 s3  = initiate_aws_s3_presigner(install_access,install_secret,install_region)
 url = s3.presigned_url( :get_object, bucket: install_bucket, key: install_key )
 return url
end

# Get public URL for S3 bucket item

def get_s3_bucket_public_url(install_access,install_secret,install_region,install_bucket,install_key)
 s3  = initiate_aws_s3_resource(install_access,install_secret,install_region)
 url = s3.bucket(install_bucket).object(install_key).public_url
 return url
end

# Show URL for S3 bucket

def show_s3_bucket_url(install_access,install_secret,install_region,install_bucket,install_key,install_type)
  if install_type.match(/public/)
    url = get_s3_bucket_public_url(install_access,install_secret,install_region,install_bucket,install_key)
  else
    url = get_s3_bucket_private_url(install_access,install_secret,install_region,install_bucket,install_key)
  end
  handle_output(url)
  return
end
